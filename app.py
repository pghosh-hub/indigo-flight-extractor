...
st.title("IndiGo Screenshot → Excel")
...
def parse_block_style(text: str) -> List[Dict]:
    ...
    for blk in parts:
        if not blk.strip():
            continue
        if not re.search(r'6E|Indigo|layover|RAIPUR|KOLKATA|HANOI', blk, re.I):
            # still attempt parse
            pass
        # flight no
        ...
        row = {
            ...
            "Departure": dep,
            "Arrival": arr,
            "Duration": "",  # block style sometimes displays duration; can be parsed further
            "Layover Time": lay
        }
        rows.append(row)
    return rows
